...
class Bsyuki {
  ...
  Bsyuki(float m, float x, float y, String BSstr, int abc) {
    ...
    mass = m + random(0.6, 2.5);

    if (abc == 1) {
      location = new PVector(x, y);
    } else if (abc ==0) {
      location = new PVector(int(random(-1, 1367)), random(-300, 0));
    }
  ...
  boolean checkTree(Bsyuki s1, int x, int y, float stop) { 
    if (stop == 1) {
      if (s1.location.x > x - 3.5 && s1.location.x < x + 3.5 && s1.location.y > y  - 2 && s1.location.y < y + 2) {
        return true;
      } else {
        return false;
      }
  ...
  boolean checkEli(Bsyuki s1, float x, float y, float stop) { 
    if (stop == 1) {
      if (s1.location.x > x - 4 && s1.location.x < x + 4 && s1.location.y > y  - 1 && s1.location.y < y + 1) {
        return true;
      } else {
        return false;
      }
